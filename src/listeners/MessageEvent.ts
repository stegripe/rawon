    ...
    public async execute(message: IMessage): Promise<any> {
        ...
        if ((await this.getUserFromMention(message.content))?.id === message.client.user?.id) {
            return message.channel.send(
                createEmbed("info", `Hi 👋 my name is **${this.client.user!.username}**, a simple Discord music bot, use **\`${this.client.config.prefix}help\`** to have a look with my commands`)
            );
        }
    }
