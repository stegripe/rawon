...
export default class RepeatCommand extends BaseCommand {
    ...
    public execute(message: IMessage, args: string[]): any {
        ...
        if (mode === "all" || mode === "queue" || mode === "*" || mode === "2") {
            ...
            return message.channel.send(new MessageEmbed().setDescription("🔁  **|** Repeating all musics in the queue.").setColor(this.client.config.embedColor));
        } else if (mode === "current" || mode === "one" || mode === "musiconly" || mode === "1") {
            message.guild.queue.loopMode = 1;
            return message.channel.send(new MessageEmbed().setDescription("🔂  **|** Repeating this music from the queue.").setColor(this.client.config.embedColor));
