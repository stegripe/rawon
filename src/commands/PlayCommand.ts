...
export default class PlayCommand extends BaseCommand {
    public constructor(public client: Disc_11, public readonly path: string) {
        super(client, path, { aliases: ["p", "music", "play-music", "add"] }, {
            name: "play",
    ...
    public async execute(message: IMessage, args: string[]): Promise<any> {
        ...
        if (message.guild?.queue !== null && voiceChannel.id !== message.guild?.queue.voiceChannel?.id) {
            return message.channel.send(new MessageEmbed()
                .setDescription(`Music on this server is currently playing to: **\`${message.guild?.queue.voiceChannel?.name}\`** voice channel`)
                .setColor("YELLOW"));
        }
        ...
        if (/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/.exec(url)) {
            ...
            message.channel.send(new MessageEmbed().setDescription(`Adding all musics from **[${playlist.title}](${playlist.url})**, hang on...`).setColor(this.client.config.embedColor))
            ...
            return message.channel.send(new MessageEmbed().setDescription(`All musics from **[${playlist.title}](${playlist.url})**, has been added to the queue`).setColor(this.client.config.embedColor));
        }
        try {
            ...
            try {
                ...
                if (videos.length === 0) return message.channel.send(new MessageEmbed().setDescription("Could not obtain any search results.").setColor("RED"));
                let index = 0;
                ...
                try {
                    ...
                    msg.delete().catch(e => this.client.logger.error("PLAY_CMD_ERR:", e));
                    return message.channel.send(new MessageEmbed().setDescription("No or invalid value entered, song selection canceled.").setColor("RED"));
                }
                if (response.first()?.content === "c" || response.first()?.content === "cancel") {
                    return message.channel.send(new MessageEmbed().setDescription("Song selection canceled.").setColor("YELLOW"));
                }
                ...
                return message.channel.send(new MessageEmbed().setDescription("Could not obtain any search results.").setColor("RED"));
            }
        }
    ...
    private async play(guild: IGuild): Promise<any> {
        ...
        if (!song) {
            serverQueue.textChannel?.send(
                new MessageEmbed().setDescription(`Song queue has finished, use **\`${guild.client.config.prefix}play\`** again to play more songs!`).setColor(this.client.config.embedColor)
            ).catch(e => this.client.logger.error("PLAY_CMD_ERR:", e));
