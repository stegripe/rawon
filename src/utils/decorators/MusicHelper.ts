...
export function isMusicPlaying(): any {
    return inhibit(message => {
        if (message.guild?.queue === null) return message.channel.send(new MessageEmbed().setDescription("There is nothing playing.").setColor("RED"));
    });
}
    ...
    return inhibit(message => {
        ...
        if (message.member?.voice.channel?.id !== message.guild.queue?.voiceChannel?.id) {
            return message.channel.send(
                new MessageEmbed().setDescription("You need to be in a same voice channel as mine").setColor("RED")
            );
        }
    });
}
...
export function isUserInTheVoiceChannel(): any {
    return inhibit(message => {
        if (!message.member?.voice.channel) {
            return message.channel.send(
                new MessageEmbed().setDescription("I'm sorry, but you need to be in a voice channel to do that").setColor("RED")
            );
        }
    });
}
...
export function isValidVoiceChannel(): any {
    return inhibit(message => {
        ...
        if (!voiceChannel?.joinable) {
            return message.channel.send(
                new MessageEmbed().setDescription("I'm sorry, but I can't connect to your voice channel, make sure I have a proper permissions!").setColor("RED")
            );
        }
        if (!voiceChannel.speakable) {
            voiceChannel.leave();
            return message.channel.send(new MessageEmbed().setDescription("I'm sorry, but I can't speak in this voice channel, make sure I have a proper permissions!")
                .setColor("RED"));
        }
    });
}
